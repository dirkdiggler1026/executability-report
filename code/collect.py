...
from rhchain import QUOTES, STOCKS, STOCK_DEC, best_quote    # noqa: E402

ROOT = Path("/root/predict-data/dexfeed_data/uniswap_v4_rh")
POOLS = Path(__file__).parent / "stock_pools.json"
...
def main() -> int:
    ...
    for sym, ps in sorted(idx.items()):
        ...
        if not base:
            out_rows.append(row(sym, "buy", 100, None, ts, ms, "",
                                st, "无法推汇率", blk, 100 * 10 ** UD, None))
            continue
        rate = base / 10 ** STOCK_DEC / 100          # 每美元多少代币
        for sz in SIZES:
            t0 = time.time()
            o, p, st = best_quote(ps, U, sz * 10 ** UD, bh)
            amt_in = sz * 10 ** UD
            r = row(sym, "buy", sz, o / 10 ** STOCK_DEC if o else None,
                    ts, int((time.time() - t0) * 1000),
                    p["id"] if p else "", st, "", blk, amt_in, o)
            if o:
                got = o / 10 ** STOCK_DEC
                r["eff_price"] = sz / got                 # 每代币实付美元
                r["impact_pct"] = (got / (rate * sz) - 1) * 100
            out_rows.append(r)

            amt = int(rate * sz * 10 ** STOCK_DEC)
            t0 = time.time()
            o2, p2, st2 = best_quote(ps, STOCKS[sym], amt, bh)
            r2 = row(sym, "sell", sz, o2 / 10 ** UD if o2 else None,
                     ts, int((time.time() - t0) * 1000),
                     p2["id"] if p2 else "", st2, "", blk, amt, o2)
            if o2:
                back = o2 / 10 ** UD
                r2["eff_price"] = back / (rate * sz)      # 每代币实收美元
                r2["recovery_pct"] = back / sz * 100      # 收回名义金额的 %
            out_rows.append(r2)
    ...
    print(f"{time.strftime('%Y-%m-%d %H:%M:%S', time.gmtime(ts))} | 块 {blk:,} | "
          ...
          + f" | roundKeccak {rk[:18]}…")
    if c.get("rpc_error"):
        print(f"   {c['rpc_error']} 行是 RPC 故障，不是流动性枯竭 —— 分析时必须剔除",
              flush=True)
    return 0
